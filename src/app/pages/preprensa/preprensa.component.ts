...
export default class PreprensaComponent
  ...
  implements OnInit
{
  ...
  public mostrarModalLiberacion = computed(
    () => this._ordenLiberacion() !== null
  );
  public mostrarCheckList = computed(
    () => this._checkListService.checkList() !== null
  );
  public puedeDefinirOrdenMetrics = computed(
    () => this.ordenMetricsPorDefinir() !== null
  );
  ...
  public ordenLiberacion = computed(
    () => this._ordenLiberacion()?.NoOrden || ''
  );
  ...
  public catalogoTiposProductos = computed(() =>
    this._metricsService.TipoMateriales()
  );
  ...
  constructor() {
    ...
    if (this._prePrensaService.ordenPorDefinir()) {
      this.cargarInformacionPorQueryParam(
        this._prePrensaService.ordenPorDefinir()
      );
    }
    ...
    effect(async () => {
      if (this._checkListService.checklistSaved()) {
        ...
        const response = await firstValueFrom(
          this._metricsService.listar(this.verPendientes())
        );
  ...
  cargarInformacionPorQueryParam = async (orden: string | null) => {
    if (orden) {
      const response = await firstValueFrom(
        this._metricsService.buscarPorPatron(orden)
      );
      const ordenMetrics = response.ordenes.find((o) => o.NoOrden === orden);
      if (ordenMetrics) {
        const existe = this.catalogoTiposProductos().some(
          (tp) => tp.value === ordenMetrics?.TipoProd
        );
        this.ordenMetricsPorDefinir.set({
          ...ordenMetrics,
          TipoProd: existe ? ordenMetrics.TipoProd : null,
        });
        this.onSelectOrder(ordenMetrics);
      } else {
        this._uiService.mostrarAlertaError(
          'Orden no encontrada',
          `No se encontró la orden con número ${orden}. Por favor, verifique el número e intente nuevamente.`
        );
      }
    }
  };
  ...
  async descargarPDF(data: any) {
    const { NoOrden } = data;
    const { infoLiberacion, sobreContenido } = await firstValueFrom(
      this._metricsService.obtener(NoOrden)
    );

    const { usuarioLibero } = infoLiberacion!;
    this._pdfService.descargarMarbetePDF(
      data,
      sobreContenido,
      usuarioLibero || ''
    );
  ...
  async cargarInformacion() {
    ...
    try {
      ...
      this._uiService.mostrarAlertaError(
        ...
        'No se pudo cargar la información de las órdenes de métricas. Por favor, inténtelo más tarde.'
      );
    } finally {
  ...
  async ir(
    ordenMetrics: OrdenMetrics,
    actual: { id_checkActual: string; liberacion?: Date }
  ) {
  ...
  async guardarOrdenMetricsPorDefinir() {
    ...
    try {
      ...
      this._uiService.mostrarAlertaError(
        'Error al guardar la orden de métricas',
        'No se pudo guardar la orden de métricas. Por favor, inténtelo más tarde.'
      );
    } finally {
